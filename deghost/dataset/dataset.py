...
class SIG17_Training_Dataset(Dataset):
    ...
    def __init__(self, root_dir, sub_set, is_training=True, is_downscale=False):
        ...
        self.scenes_list = sorted(os.listdir(self.scenes_dir))#[:12800]
    ...
    def __getitem__(self, index):
        ...
        if self.is_downscale:
            C,H,W = img0.shape # 6, H, W
            # transform = T.Resize((H//2,W//2))
            img0 = F.resize(img0, (H//2,W//2))
            img1 = F.resize(img1, (H//2,W//2))
            img2 =F.resize(img2, (H//2,W//2))
            label = F.resize(label, (H//2,W//2))
            # print(img0.shape)
...
class Img_Dataset(Dataset):
    def __init__(self, ldr_path, label_path, exposure_path, patch_size, is_downscale=False):
        if is_downscale:
            ldr = read_images(ldr_path)
            ldr = torch.from_numpy(np.transpose(ldr, (0,3,1,2)))
            N,C,H,W = ldr.shape
            # print(ldr.shape)
            self.ldr_images = np.transpose(F.resize(ldr, (H//2,W//2)).detach().numpy(),(0,2,3,1))

            origin = read_label(label_path, 'HDRImg.hdr')
            origin = torch.from_numpy(np.transpose(origin, (2,0,1)))
            C,H,W = origin.shape
            # print(origin.shape)
            self.label = np.transpose(F.resize(origin, (H//2,W//2)).detach().numpy(),(1,2,0))
        else:
...
def SIG17_Test_Dataset(root_dir, patch_size, is_downscale=False):
    ...
    for ldr_dir, label_dir, expo_times_dir in zip(ldr_list, label_list, expo_times_list):
        yield Img_Dataset(ldr_dir, label_dir, expo_times_dir, patch_size, is_downscale)
