...
from dataset.dataset import SIG17_Test_Dataset
...
def main():
    ...
    # model architecture
    model_dict = {
        0: HDRTransformer(32)#(embed_dim=60, depths=[6, 6, 6], num_heads=[6, 6, 6], mlp_ratio=2, in_chans=6),
    }
    ...
    for idx, img_dataset in enumerate(datasets):
        ...
        if args.save_results:
            ...
            print(pred_img_mu.shape)
            imageio.imwrite(os.path.join(args.save_dir, '{}_tm.png'.format(idx)), 
                            np.uint8(pred_img_mu[:,:,[2,1,0]]))
