...
import dataset
#import runs.ours_4.network as network
import network_cnn as network
...
def main_worker(rank, world_size, args):
    ...
    if args.burst:
        test_dataset_dir = '/workspace/dataset/burst_input/1110_1/'
    ...
    while train_state:
        ...
        with torch.no_grad():
            ...
            for data_group in testing_generator:
                ...
                # For the comparison
                if 'cnn' in args.result_dir:
                    print('cnn')
                    b,c_h,c_w,c = input_bayer.shape
                    input_bayer_l = input_bayer[:,:,:c_w//2,:]
                    input_bayer_r = input_bayer[:,:,c_w//2:,:]
                    pred_hdr_l = net(input_bayer_l).permute(0,2,3,1)
                    pred_hdr_r = net(input_bayer_r).permute(0,2,3,1)
                    pred_hdr = torch.zeros((b,c_h,c_w,3))
                    pred_hdr[:,:,:c_w//2,:] = pred_hdr_l
                    pred_hdr[:,:,c_w//2:,:] = pred_hdr_r
                else:
                ...
                if save_result:
                    print(f"{input_name}")
                    if args.burst:
                        make_dir(f'./{args.result_dir}/burst_result/1110_1')
                        imageio.imwrite(f'./{args.result_dir}/burst_result/1110_1/hdr_{input_name}.hdr', pred_hdr[0,:,:,:].detach().cpu().numpy())
                        imageio.imwrite(f'./{args.result_dir}/burst_result/1110_1/tm_{input_name}.png',
                ...
                if save_result:
                    print(f"{input_name}:{cur_psnr_mu}")
            ...
            if args.burst:
                print("Successfully save burst result images")
                return        
