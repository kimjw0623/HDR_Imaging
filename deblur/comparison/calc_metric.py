...
for img_name in test_name:
    gt_ldr = imageio.imread('C:\\Users\\user\\Desktop\\ablation\\hdr_noise_0.0_0.5\\gt_ldr_{}.png'.format(img_name))
    method_ldr = imageio.imread('C:\\Users\\user\\deblurring_dataset\\comparison\\serrano_result\\{}.png'.format(img_name))
    
    psnr = color_psnr(method_ldr,gt_ldr,255)
    print(psnr)
    
    mu_psnr += psnr
    mu_ssim += ssim(method_ldr,gt_ldr)
...
#     gt_ldr = (gt_ldr * 255).astype('uint8')
...
print(f'PSNR: {mu_psnr/200}')
print(f'SSIM: {mu_ssim/200}')